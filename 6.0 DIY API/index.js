...
app.get("/filter", (req, res) => {
  ...
  const filtered = jokes.filter((joke) => joke.jokeType === type);
  res.json(filtered);
});
...
app.put("/jokes/:id", (req, res) => {
  ...
  const index = jokes.findIndex((joke) => joke.id === newId);
  jokes[index] = newJoke;
  res.json(newJoke);
});
...
app.patch("/jokes/:id", (req, res) => {
  let newId = parseInt(req.params.id);
  const current = jokes.find((joke) => joke.id === newId);
  const newJoke = {
    id : newId,
    jokeType : req.body.type || current.jokeType,
    jokeText : req.body.text || current.jokeText,
  };
  const index = jokes.findIndex((joke) => joke.id === newId);
  jokes[index] = newJoke;
...
app.delete("/jokes/:id", (req, res) => {
  ...
  if(index > -1){
    ...
    res.status(400).json({
      error : `joke with ${newId} not found. No jokes were deleted.`
    });
  }
});
